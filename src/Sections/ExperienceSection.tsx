...
export const ExperienceSection = (props: ExperienceSectionProps) => {
    ...
    return (
        <div id={sectionId} className="experienceSectionContainer">
            {entries.map((experience: Experience, index) => {
                return (
                    <ExperienceItem key={`${experience.title}_${index}`} {...experience} />
                )
            })}
        </div>
    )
            ...
            <div className="experienceContainer__content">
                ...
                <div className={'singleTagContainer'}>
                    {tags.map((tag: string, index) => {
                        return (
                            <SingleTag key={`${tag}_${index}`}name={tag} />
                        )
                    })}
                </div>
            </div>
...
const SingleTag = (props: { name: string }) => {
    const { name } = props;
    return (
        <div className="singleTagContainer__tag">
            {name}
        </div>
    )
}